 ...
 * Handles terminal resize events via useStdoutDimensions hook
 */
export const TokenUsageChart = React.memo(() => {
	...
	const tokenValues = useMemo(() => tokenTimeline.map((point) => point.running_total || 0), [tokenTimeline])
	...
	return (
		<Box flexDirection="column" gap={1}>
			...
			<Box borderStyle="round" borderColor={theme.ui.border.default} paddingX={1}>
				<Sparkline data={throttledValues} width={sparklineWidth} height={8} colorScheme="blue" mode="block" />
			</Box>
		</Box>
	)
})
